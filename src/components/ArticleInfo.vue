<template>
  <div class="article-info">
    ...
    <!-- 正文内容 -->
    <scroll class="container" @click="closeInput" :noMore="true" :pullup="false">
      <h2 class="article-title" @click="closeInput">{{articleTitle}}</h2>
      <p class="article-hit">阅读量： {{hit}}</p>
...
import $ from 'jquery'
import { GetArticleInfo, GetReviewFront, AddReview, AddReviewReply, AddReviewLike, GetArticleLike } from '@/api/index'
...
export default {
  ...
  methods: {
    ...
    isShare () {
      ...
      let w = window.location.href + '&share'
